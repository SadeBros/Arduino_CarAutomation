...
#define SMOOTH_CALCULATION_FREQUENCY_NUMBER 8                       //  Reads eight times the voltage value from Arduino Analog Pin 4 ...
                                                                    //  ...in every 50 ms and then returns the average of total measurement.
...
void loop(){
  if ( RTC.alarm( ALARM_2 ) ) //  If Real Time Clock produces an interrupt because of ALARM_2 register.
  {
    do{
      ...
      else if( triggeredAlarm.repeat() == 1 ){      //  triggeredAlarm repeats daily.
...
//  Reads eight times the voltage value from Arduino Analog Pin 4 ...
//  ...in every 50 ms and then returns the average of total measurement.
float SmoothVoltageCalculation(){
  float total = 0;
